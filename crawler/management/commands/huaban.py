...
def incrementStr(string):
	...
	for i in bstr[::-1]:
		if i == 122:
			...
			i = increment[increment.index(i)+1]
			result.insert(0, i)
			break
...
for i in range(2, 300):
	code = 'jfqv3nj8'
	pics = requests.get('http://huaban.com/explore/qingchunkeaimeinv/?{}&page={}&per_page=20&wfl=1'.format(code, i), headers=headers).json()['pins']
	code = incrementStr(code)
	for pic in pics:
		filename = pic['file']['id']
		imgBinary = requests.get('http://img.hb.aicdn.com/' + pics['file']['key'], stream=True).content
